 ...
 * WeatherDataParser parse Weather input data by considering 2 input files as timezoneFileLookupPath, weatherDataInputPath
 * and generates WeatherDataDto
 */
public class WeatherDataParser {

	/**
	 * parseWeatherTimezoneRecord parse the weather data input line by lookup timezone map
	 * and generates the WeatherDataDto with required parameters
	 * @param weatherline
	 * @param tzlonglatMap
	...
	/**
	 * parseWeatherRecord parse weather data input line and generated WeatherDataDto 
	 * @param weatherline
	 * @return WeatherDataDto
	 */
	public WeatherDataDto parseWeatherRecord(String weatherline) {
		String[] weathersplit = weatherline.split(WeatherConstants.ROW_SPLIT_SEPERATOR);
	...
	/**
	 * parseTimeZoneRecord lookup data into timezone file and generated the required fields in WeatherDataDto
	 * @param longlatrec
	 * @param objWeatherDto
	 * @return WeatherDataDto
	 * @throws ParseException
	...
	public WeatherDataDto parseTimeZoneRecord(String longlatrec,
		...
		String[] longlatsplit = longlatrec.split(WeatherConstants.ROW_SPLIT_SEPERATOR);
		...
		SimpleDateFormat dateformater = new SimpleDateFormat(WeatherConstants.TIME_yyyy_MM_dd_T_HH_mm_ss_Z);
		dateformater.setTimeZone(TimeZone.getTimeZone(WeatherConstants.TIME_UTC_TIMEZONE));
		...
		// running Operating system timezone to specified timezone in lookup timezone data
		SimpleDateFormat formatter = new SimpleDateFormat(WeatherConstants.TIME_yyyy_MM_dd_T_HH_mm_ss_Z);
		formatter.setTimeZone(TimeZone.getTimeZone(objWeatherDto.getTimezone()));
		...
		//converts localdate to LocalDateTime to find out timeoffset value in UTC
		LocalDateTime dt = LocalDateTime.of(
				Integer.parseInt(yyyyMMdd.split(WeatherConstants.DATE_FIELD_SEPERATOR)[0]),
				Integer.parseInt(yyyyMMdd.split(WeatherConstants.DATE_FIELD_SEPERATOR)[1]),
				Integer.parseInt(yyyyMMdd.split(WeatherConstants.DATE_FIELD_SEPERATOR)[2]),
				Integer.parseInt(HHmmss.split(WeatherConstants.TIME_FIELD_SEPERATOR)[0]),
				Integer.parseInt(HHmmss.split(WeatherConstants.TIME_FIELD_SEPERATOR)[1]),
				Integer.parseInt(HHmmss.split(WeatherConstants.TIME_FIELD_SEPERATOR)[2]));
